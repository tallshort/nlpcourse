...
public class runMain {
	...
	private static void excuteClusty(int classNum, String lastName) {
		...
		{
			String s="vcluster -clmethod=rb "+//采用层次聚类
			"-clustfile=matrixFile\\"+lastName+classNum+".cluster "+ //最后分类文件
	...
	private static void processAll(File[] files,String Name,int from,int end) {
		try {
            ...
            File fColLabel = new File("matrixFile\\"
                    + Name +".clabel"); // 记录列信息
            ...
            for (int i=from ;i<end;i++)
            {
                ...
                DataProcess d = new DataProcess(files[i], map, lastIndex, totalNum);// 处理每个实例
                result += d.Process() + "\n"; // 换行表示每个 实例一行
